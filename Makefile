...
all: clean
	...
	apprepo --destination=$(PWD)/build appdir boilerplate mc

	echo "exec \$${APPDIR}/bin/mc \"\$${@}\"" >> $(PWD)/build/Boilerplate.AppDir/AppRun
	...
	export ARCH=x86_64 && bin/appimagetool.AppImage $(PWD)/build/Boilerplate.AppDir $(PWD)/Mc.AppImage
	chmod +x $(PWD)/Mc.AppImage
